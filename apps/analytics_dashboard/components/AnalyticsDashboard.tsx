...
const AnalyticsDashboard: React.FC = () => {
  ...
  const response = await fetch(`/api/dashboard-data?timeframe=${selectedTimeframe}`);
      ...
      if (result.success) {
        ...
        // Show success message only if data comes from main database
        if (result.source === 'main_database') {
          console.log('✅ Real data loaded from main database');
        } else {
          console.log('⚠️ Using fallback data - main database not connected');
        }
      } else {
        // Even if not successful, we'll get fallback data
        setData(result.data);
        console.log('⚠️ Main database not available, showing empty structure');
      }
    } catch (error) {
      console.error('Error fetching dashboard data:', error);
      // Set empty data structure if API fails completely
      setData({
        peakHours: [],
        departmentalLoad: [],
        weeklyTrends: [],
